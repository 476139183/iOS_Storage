...
//        tttLabel.textColor = .black
    ...
    private func checkMarkDownURL(label: TTTAttributedLabel) {
        ...
        if let result = regex?.firstMatch(in: currentAttributedString.string, options: [], range: NSRange.init(location: 0, length: currentAttributedString.string.count)) {
            ...
            label.setText(currentAttributedString)
            
            label.linkAttributes = [
                kCTForegroundColorAttributeName: UIColor.green,
                NSAttributedString.Key.underlineColor: UIColor.colorWithHexString("#797F8C"),
...
//                NSAttributedString.Key.underlineColor: UIColor.colorWithHexString("#797F8C"),
//                NSAttributedString.Key.underlineStyle: NSNumber(value: NSUnderlineStyle.single.rawValue),
//            ], textCheckingResult: NSTextCheckingResult.linkCheckingResult(range: newRange, url: URL.init(string: linkURL)!)))
            // add link
            
            
            label.addLink(with: NSTextCheckingResult.linkCheckingResult(range: newRange, url: URL.init(string: linkURL)!))
